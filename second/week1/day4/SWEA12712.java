...
public class SWEA12712 { //TC: 6/10
	...
	private static int calculatePlusShape(int M, int row, int col) {
		...
		for(int k = 1; k < M; k++) {
			sum += sumByDelta(k, row, col, pdx, pdy);
		}
	...
	private static int calculateCrossShape(int M, int row, int col) {
		...
		for(int k = 1; k < M; k++) {
			sum += sumByDelta(k, row, col, cdx, cdy);
		}
		
		return sum;
	}

	private static int sumByDelta(int k, int row, int col, int[] dx, int[] dy) {
		int rslt = 0;
